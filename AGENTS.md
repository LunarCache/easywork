...
- `packages/core/src/server/guarded-stream.ts` — **Guarded Stream** 深模块：统一 Fastify hijack / 流式响应头、客户端断开 → `AbortSignal`、安全写入、幂等结束、心跳与 cleanup；Agent / 模型下载 / Inbox / `/v1` adapters 只保留各自的协议 framing。
...
6. **Guarded Stream 单一连接生命周期**：所有流式 HTTP 写口（`/agent/run`、模型下载、`/inbox/events`、`/v1` 本地透传 / 云端 / fallback）必须经 `createGuardedStream` 统一 hijack、响应头、断连取消、安全写入、幂等结束、心跳与 cleanup；route / 协议翻译器仍拥有具体 framing，不得重新直接操作 `reply.raw` 生命周期。
...
- **统一 npm**（环境无 pnpm）。
- **测试 416 通过**（vitest；另 1 个真机 e2e 默认 skip）。另有 **Playwright UI e2e 48 条** 作为 CI 主跑层（真 daemon + 真 Vite + 隔离 data dir），以及 Windows NSIS 构建 + SEA `/health` 冒烟作为发布关键路径。`npm run lint` 当前 0 warning / 0 error。改 `@ew/core` / `@ew/sdk` 源码后，依赖其 `dist` 的下游（daemon 打包内联 dist）需 `npm run build` 才生效。
...
npm test               # vitest（416 通过；另 1 个真机 e2e 默认 skip）
