...
- **记忆——抽取（模型抽取 + 批量，非每轮）**：`buildFactExtractor` 用对话模型抽取（提示词按 scope 切换：工作区盯变动/约束/坑且含写文件/命令的工具摘要，对话盯身份/偏好/技巧）。调度由 **`ExtractionScheduler`（SessionHost 拥有，非 pi 扩展闭包）** 负责：扩展每轮把对话转发给它，**增量缓冲**自上次抽取以来的新轮次（列表变短=压缩→重新基线，长突发也不漏早期轮次），缓冲达 `maxTurns(24)` 立即分块抽、否则 **~90s 空闲去抖**。`core.stop()` 在**停模型前** `flushAllExtraction()`（graceful 退出不丢尾部；SIGKILL 仍会丢，可接受）；**删会话则 discard 不抽**（不把将删的对话写进记忆）。压缩不再触发抽取（缓冲独立于 pi 上下文，避免与主运行并发打模型）。
