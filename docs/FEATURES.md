...
- **工作台面板**（对话区与工作区共用）：打开即显示**改动〔工作区〕/ 文件 / 终端 / 浏览器**常驻模式栏，不经过二级启动菜单；宽屏为可拖拽并持久化宽度的右侧列，窄屏保留入口并切成右侧浮层。文件模式在普通侧栏使用“列表 → 独占详情”的主从导航，放大态使用左侧文件导航 + 右侧预览双栏；文件名、预览 / 源码和复制 / 下载只保留 FileViewer 的一套工具栏。顶部保留放大 / 关闭，macOS 放大态复用标题栏 traffic-light 安全区。
