...
- **思考能力（分级）**：云端可思考模型走 pi `setThinkingLevel`（真分级）；本地经 llama.cpp 请求体注入（`enable_thinking` + `thinking_budget_tokens`）。**自动重试**（provider 抖动退避重试，状态条提示）、**prompt caching**（云端 `cacheRetention=long` + 会话级缓存）默认开启；**上下文压缩**自动阈值触发或 `/compact` 手动触发，进度经状态条提示。
