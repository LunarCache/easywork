...
- **真实工具审批流（4 档）**：`read-only` / `approve-each` / `auto-edits` / `full-auto`，经 pi `tool_call` 钩子映射；危险工具经 SSE 挂起，UI 在**对话栏上方弹出审批卡**（内嵌、非遮罩弹层）「允许 / 总是允许 / 拒绝」。
...
- **展开式侧栏**：顶部快捷操作（新对话 ⌘N / 打开工作区 / 收件箱）+「项目」分区（工作区折叠组，CWD 角标 / 文件树 / 删除）+「对话」分区（独立聊天）+ 底部设置 + 连接状态点。
- **对话区**：用户 `acc-weak` 软气泡右对齐 / AI 无头像纯 prose 左对齐；**行内工具调用**（Codex Flat 风：扁平左竖线、过去式步骤、零卡片零暗井，默认折叠、运行中展开）——连续只读勘探（读 / 搜 / 列 / 找）**聚合成「探索 · N」组**；**思考**左竖线 accent；**编辑**〔过去式「已编辑」+ 文件类型徽标 + 文件名(亮)/目录(灰) + `+/-`〕展开为**语法高亮 diff**（复用 `.hljs-*` 着色 + 双栏行号〔解析 `@@`〕 + 改动行红/绿竖条）；**运行**〔`SquareTerminal` 图标 + 过去式「已执行」+ 折叠态命令预览〕展开为浅底圆角卡（`$ 完整命令`悬挂缩进换行 + 灰输出，同卡）+ **文件改动汇总卡**（消息底部，默认折叠、点击跳转查看 diff）+ 流式 / 引用 / HTML 工件 / 图片多模态 / 审批。
...
- **composer**：textarea + 底栏（左 `+`〔聊天=菜单收纳 思考/联网/知识库 开关；工作区=图片上传〕· **思考档位 chip**〔关 / 低 / 中 / 高，点击循环，按模型持久化〕· 工作区另有审批策略 pill〔Read only / Ask each change / Auto-edit / Full auto〕· 右 上下文用量环 · 模型选择 · 生成参数 · 圆形发送），保持单行、挤不下优雅换行。
...
- **设置（整页内嵌，非弹层）**：左导航 8 项切换——**通用**（明暗模式〔浅色 / 深色 / 跟随系统〕）/ **模型** / **知识库** / **Skills** / **MCP** / **记忆** / **向量记忆** / **本地网络**；模型 / 知识库 / Skills / MCP / 记忆 五管理页以 keep-alive 内嵌（取代旧「插件页」+ 弹窗）。通用类用卡片行（标题 + 说明左、控件右）。模型页支持**删除本地模型**（hover 显形，删的若是向量记忆引擎会一并停嵌入进程）。
