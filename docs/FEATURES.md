...
- **设置（整页内嵌，非弹层；覆盖整窗含标题栏）**：左导航 6 项切换——**通用** / **模型** / **知识库** / **Skills** / **MCP** / **记忆**。**统一外壳**：每页共用同一套**大标题 + 自适应留白**（横向 `clamp` 内边距随窗口缩放，切 tab 不跳动），记忆 / 知识库的图标副标题头并入统一标题。**通用**用卡片行（标题 + 说明左、控件右）——目前仅**界面主题**（下拉：浅色 / 深色 / 跟随系统）。模型 / 知识库 / Skills / MCP / 记忆 五管理页以 keep-alive 内嵌（取代旧「插件页」+ 弹窗）；**模型页（本地）**顶部含**网络访问**卡——暴露到局域网为**开关**（开=0.0.0.0 须 api-key、关=仅本机）+ API Key + 已加载模型端点列表（把模型服务 llama router + `/v1` 网关暴露给局域网，故归入模型页）；**记忆**工具栏自带向量召回状态 + 启用（原「向量记忆」页已并入）。左导航宽度跟随主侧栏（分割线对齐）。模型页支持**删除本地模型**（hover 显形，删的若是向量记忆引擎会一并停嵌入进程）。
