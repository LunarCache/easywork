...
- **composer**：textarea + 底栏（左 `+`〔聊天=菜单收纳 思考/联网/知识库 开关；工作区=图片上传〕· 工作区另有审批策略 pill〔Read only / Ask each change / Auto-edit / Full auto〕· 右 上下文用量环 · 模型选择 · 生成参数 · 圆形发送），保持单行、挤不下优雅换行。
