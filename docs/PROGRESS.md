...
## 2026-06-28 — CI 友好的 Playwright UI e2e 扩容到 13 条
...
- **CI workflow 收口**：`.github/workflows/ci.yml` 从只跑 `settings.spec.ts` 升级为直接跑 `npm run test:e2e`，使整组 13 条 UI e2e 成为默认主跑层。
- **验证**：`npm run test:e2e -- apps/ui/e2e/settings.spec.ts apps/ui/e2e/composer.spec.ts apps/ui/e2e/navigation.spec.ts apps/ui/e2e/files-memory.spec.ts apps/ui/e2e/kb-skills.spec.ts` → **13 passed**。
