...
**鉴权 / CORS**：先 CORS 钩子（必须先于鉴权；OPTIONS 直接 204），再 Bearer-token 钩子守住管理面与内部 API。例外只有 `/health` 和外部渠道 webhook：`/im/:id/webhook` 不要求 EasyWork 内部 Bearer，因为真实平台回调通常不能带该 header；平台签名/secret 校验由 adapter 负责。body 上限 32MB。
