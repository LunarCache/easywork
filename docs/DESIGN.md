...
- **IM 连接器**（Telegram / Feishu / WeChat 等）——外部渠道汇入同一个大脑；
...
│ IM 连接器 (Telegram/Feishu/WeChat…) │ ─▶ │ RouterServerManager(单 llama serve 多模型路由)  │
...
`ProviderManager`（`providers/manager.ts`）统一处理所有云端，无每厂商代码。`CloudProviderConfig = {id, kind, baseUrl?, api?, apiKey?, headers?, modelConfigs[]}`，每个 declared model 暴露为 provider-scoped route id（例如 `provider:my-deepseek:deepseek-v4`），但 `OpenAICompatibleEngine.mapModelId` 会把请求体里的 `model` 还原成上游真实模型名。`modelSources` 额外带 `modelId` 供 UI 显示。内置 provider 与自定义 provider 即使声明同名模型，也不会覆盖上下文窗口或路由。配置序列化进 SQLite `providers` 键，开机恢复。
...
`GET /models` 回 `endpoints`（router 直连 baseUrl）+ `bindHost` + `lanIp`，供外部客户端直连或经 `/v1`。
...
- **Workspace**：`ContextBar` 除项目 / 分支下拉外，还接收 children，把 **模型 / 思考 / 上下文压力**注入到同一行；底栏左侧 `+` 同样只做**上传图片**，右侧保留**审批策略 pill**（中文档位 只读/逐项确认/自动编辑/完全访问，full-auto 橙色警示；改档 PATCH `project.approvalMode`，`send()` 前 await 在途 PATCH 防"切档即发"）与发送。**空态居中**（`msgs.length===0` 渲染 `.ws-hero`：品牌标 + 时段问候语 + 居中限宽 composer，否则消息流在上、composer 在底）。SideDock 带 git 上下文；mutating 工具后刷 git + ws 文件 + 上报分支给标题栏。
...
- **思考分级**：思考状态统一以前置 pill 呈现（关/低/中/高，点击循环）+ `/think`；`thinkingLevel` **按模型持久化**（`prefs.ts`，默认 off），run 时随 `runAgent` 下发。
...
- **两类区**：通用是**卡片行**（`.set-group`/`.set-row`，标题+说明左、控件右，ZCode 式对齐）——目前仅界面主题，为**自绘下拉**（`AppearanceSelect`：按钮 + `.set-select-menu` 深色弹层 + `menu-backdrop`——原生 `<select>` 的选项菜单是 OS 浅色绘制、无法随主题着色，故自绘）；模型 / 知识库 / Skills / MCP / 记忆 五管理页以 **lazy keep-alive 内嵌**（访问过的 `.set-pane` `display:none` 保活，避免 KB 上传/索引轮询中断）。**网络访问**（暴露到局域网 `.set-toggle` 开关 开=0.0.0.0/关=127.0.0.1 · API Key · 已加载端点 · 0.0.0.0 警告）放在**模型页（本地 tab）顶部**——它暴露的是模型服务（llama router + `/v1` 网关），故归模型而非通用（`changeBind`/`endpointUrl` 随之迁入 `Models.tsx`）。**向量召回**控件原独立「向量记忆」页已删除——记忆页（`MemoryOverlay`）工具栏本就自带状态点 + `向量召回·N维` / `未启用 + 启用`。取代旧「插件页」+ 弹窗。
...
- **模型**：本地/云端分页；本地按 kind（文本/嵌入/视觉）显示，load/unload 或"启用向量"，文本/视觉模型卡片可展开**运行参数**面板（预设 + `temperature/top_p/top_k/min_p/repeat_penalty/max_tokens`，空值=引擎默认；保存到 `models.local.settings` 并由 `SessionHost` 注入聊天/工作区/渠道默认采样），**hover 显形删除钮**（删的若是当前向量记忆引擎模型→后端 `/models/local/delete` 一并停嵌入进程 + 清持久化设置）；嵌入模型删除分级提示；无 llama 二进制时一键安装 banner；HF 搜索下载（流式 %）；云端 provider 预设 + 手填上下文窗口。
