...
| `@ew/im-connectors` | Channel Gateway（adapter registry + 配置/状态/webhook/allowlist）与平台连接器（Telegram long-poll、Feishu/Lark WebSocket 默认 + webhook 高级模式已实现，Discord/WeCom 待补）。 |
...
- **`ChannelAdapterRegistry`**（`registry.ts`）：注册平台 adapter；当前内置 Telegram 与 Feishu/Lark，后续 Discord/WeCom 在这里扩展。
...
**已实现 = Telegram adapter + Feishu/Lark adapter**。Telegram 走纯 HTTP Bot API long-poll；多数 IM 不能逐 token 流，所以回复先聚合再发，停止走 `AbortController`，保存/删除/退出不等待默认 25 秒轮询自然返回。Feishu/Lark 默认走官方 SDK WebSocket 长连接，设置页提供扫码创建/授权应用，不要求公网地址；高级 webhook 模式继续支持 URL verification、Verification Token、`X-Lark-Signature`、Encrypt Key 加密回调解密、`im.message.receive_v1` 文本事件归一化与 `tenant_access_token` 文本回复。Feishu/Lark adapter 在非 `transport:webhook` 时拒绝 public webhook；webhook 模式必须配置 `verificationToken` 或 `encryptKey`，否则 401。**WeCom/Discord 待补**。**个人微信无官方机器人 API → 仅实验性占位、默认关闭**；只做企业微信（WeCom）。
...
- **IM**：`packages/im-connectors/src/{adapter,registry,gateway,host,telegram,feishu,index}.ts`
