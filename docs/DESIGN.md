...
- **展开式侧栏**：快捷操作（新对话 ⌘N / 打开工作区 / 收件箱）+「项目」折叠组（CWD 角标 / 文件树 / 删除 / 新建会话）+「对话」分区 + 底部设置 + 连接状态点。可拖拽列宽（200–460，持久化）。
...
- **Workspace**：composer 有**审批策略 pill**（改档 PATCH `project.approvalMode`，`send()` 前 await 在途 PATCH 防"切档即发"）+ 思考档位 chip；SideDock 带 git 上下文；mutating 工具后刷 git + ws 文件 + 上报分支给标题栏。
...
整页内嵌（非弹层），左导航 8 项：**通用 / 模型 / 知识库 / Skills / MCP / 记忆 / 向量记忆 / 本地网络**。
- **两类区**：通用 / 向量记忆 / 本地网络是**卡片行**（`.set-group`/`.set-row`，标题+说明左、控件右，ZCode 式对齐）；模型 / 知识库 / Skills / MCP / 记忆 五管理页以 **lazy keep-alive 内嵌**（访问过的 `.set-pane` `display:none` 保活，避免 KB 上传/索引轮询中断）。取代旧「插件页」+ 弹窗。
