...
`resolveModel`（`session-host.ts`）：**先本地**（`local.baseUrlFor(modelId)` 命中 → `provider:"local"`, `input:["text","image"]`, ctx 来自 `local.contexts()`），**再云端**（`providers.findByModel` → pi `ModelRegistry`）。本地 server 设了 `--api-key`（0.0.0.0 暴露）时把 key 注入 pi auth，连回环也带 Bearer。
...
- **Chat**：composer 输入卡顶部用 `ComposerContextStrip` 前置本轮生效的 **模型 / 思考 / 联网 / 知识库 / 上下文压力**；底栏左侧 `+` 仅做**上传图片**，右侧保留生成参数与发送。历史工件仍走 SideDock（fs 工具后刷新、首件自动开 dock）。
