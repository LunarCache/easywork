...
| `@ew/im-connectors` | Channel Gateway（adapter registry + 配置/状态/webhook/allowlist）与平台连接器（Telegram 已实现，Discord/WeCom/飞书待补）。 |
...
- **`ChannelAdapterRegistry`**（`registry.ts`）：注册平台 adapter；当前内置 Telegram，后续 Discord/WeCom/Feishu 在这里扩展。
...
管理面都走 daemon Bearer；webhook 入口不走内部 Bearer，而是交给 adapter 做平台级校验。设置页的渠道类型从 `/im/adapters` 元数据生成，只暴露已注册的平台，避免用户保存未实现 adapter。

**已实现 = Telegram adapter**（纯 HTTP Bot API，long-poll；多数 IM 不能逐 token 流，所以回复先聚合再发）。Telegram long-poll 停止走 `AbortController`，保存/删除/退出不等待默认 25 秒轮询自然返回。**WeCom/飞书/Discord 待补**。**个人微信无官方机器人 API → 仅实验性占位、默认关闭**；只做企业微信（WeCom）。
...
- **IM**：`packages/im-connectors/src/{adapter,registry,gateway,host,telegram,index}.ts`
