...
共用 MessageStream / SideDock / ModelSelect / composer / 审批卡 / **斜杠命令面板** / **思考分级 chip**。
- **Chat**：composer `+` 菜单（上传图片 + 联网/知识库 开关，KB 开可选集合）+ 思考档位 chip；右侧 ContextRing + ModelSelect + 参数 + 发送；工件面板（fs 工具后刷新、首件自动开 dock）。
- **Workspace**：composer 有**审批策略 pill**（中文档位 只读/逐项确认/自动编辑/完全访问，full-auto 橙色警示；改档 PATCH `project.approvalMode`，`send()` 前 await 在途 PATCH 防"切档即发"）+ 思考档位 chip + **内嵌 ModelSelect**（移出原顶部 ws-bar，底栏右侧）；**空态居中**（`msgs.length===0` 渲染 `.ws-hero`：品牌标 + 时段问候语 + 居中限宽 composer，否则消息流在上、composer 在底）；**上下文条 `ContextBar`**（输入卡上方，项目/分支两个可选下拉：搜索过滤 + 列表 + 页脚动作，`menu-backdrop` 点击关；分支取 `gitBranches.all`、当前显示未提交改动数、`gitSwitch` 切换、「Git 图谱」开 dock）。SideDock 带 git 上下文；mutating 工具后刷 git + ws 文件 + 上报分支给标题栏。
- **审批卡**（`.approval-wrap`）：危险工具挂起时在 **composer 正上方内嵌弹出**（与输入框同宽、从下滑入），非遮罩弹层——不挡对话；含工具名 + 命令 JSON + 拒绝/本会话总是允许/允许。Chat 与 Workspace 一致。
- **斜杠命令**（`lib/slash.ts` + `components/SlashPalette.tsx` 的 `useSlashPalette` hook，两 composer 共用）：输入「/」弹两阶段自动补全（命令名→参数）；`onKeyDown` 优先消费 ↑↓/Enter/Tab/Esc。`/think <档位>` 设思考、`/model <名>` 切模型（下一轮 run 带新 modelId，复用 getOrCreate 重建保上下文）、`/compact` 调 `POST /threads/:id/compact`。
- **思考分级**：composer chip（关/低/中/高，点击循环）+ `/think`；`thinkingLevel` **按模型持久化**（`prefs.ts`，默认 off），run 时随 `runAgent` 下发。
